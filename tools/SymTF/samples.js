...
// A 10-stage Sallen-Key low-pass cascade -- 50 R/C/O elements, every one its own
// symbol. It is here to show the analyzer staying FULLY SYMBOLIC on a large
// circuit: the flat H(s) of a 10-biquad chain has astronomically many terms, so
// the engine tears it at the op-amp outputs and returns it factored -- one
// second-order section per stage. Enter values (or leave them symbolic) and it
// stays a 20th-order transfer function in 40 free components.
//
// Generated by tiling one verified stage rather than hand-laying 140+ points:
// stage geometry that extracts to `Ra in A; Rb A B; Ca A O; Cb B 0; O B O O`
// (a unity-gain Sallen-Key section), translated by a fixed pitch, with each
// stage's output wired into the next stage's input. test_samples.js checks the
// extracted result structurally (50 elements, 10 op-amps, clean chain).
(function () {
    ...
    window.Samples.active_lpf_x10 = {
        title: 'Active LPF ×10 (50 elements)',
        model: buildCascade(10),
    };
})();
