...
class BlogManager:
    ...
    def save_articles(self):
        ...
        try:
            ...
            # 記事グリッドを置換 - HTMLコメントを利用した正確なパターン
            grid_pattern = r'(<div class="articles-grid">\s*)(.*?)(\s*</div>\s*<!-- ページネーション\（下部\） -->)'
            new_articles_html = '\n'.join(article_cards)
            new_grid = f"\\g<1>\n{new_articles_html}\n\\g<3>"
            ...
            # ページネーション情報を更新
            self.update_pagination_info(new_content)
    ...
    def update_pagination_info(self, content):
        """ページネーション情報を現在の記事数に基づいて更新"""
        try:
            total_items = len(self.articles)

            # 上下両方のページネーション要素のデフォルト値を設定
            pagination_updates = []

            # 上部ページネーション情報の更新
            if 'paginationInfoTop' in content:
                pagination_updates.append(('paginationInfoTop', f'1-{min(10, total_items)} / {total_items}件'))

            # 下部ページネーション情報の更新
            if 'paginationInfoBottom' in content:
                pagination_updates.append(('paginationInfoBottom', f'1-{min(10, total_items)} / {total_items}件'))

            # 旧形式のページネーション情報の更新（後方互換性）
            if 'paginationInfo' in content and 'paginationInfoTop' not in content:
                pagination_updates.append(('paginationInfo', f'1-{min(10, total_items)} / {total_items}件'))
            ...
            if pagination_updates:
                ...
                for elem_id, text in pagination_updates:
                    # <span id="paginationInfo*">内容</span> パターンで更新
                    pattern = rf'(<span id="{elem_id}">)[^<]*(</span>)'
                    updated_content = re.sub(pattern, rf'\g<1>{text}\g<2>', updated_content)
        ...
        try:
            ...
            if article_file.exists():
                response = messagebox.askyesno("確認",
                    f"'{article['title']}'の記事ページは既に存在します。\n\n"
                    f"ファイル: {article_file}\n"
                    f"ディレクトリ: {article_slug}\n\n"
                    "上書きしますか？")
                if not response:
                    return
            ...
            # SEO関連のプレースホルダーを置換
            description = article.get('summary', article['title'] + ' - ice458の技術メモ・備忘録・修理記録など')
            if len(description) > 150:
                description = description[:147] + "..."

            keywords = ','.join(article['categories']) + ',技術メモ,備忘録,ice458'
            canonical_url = f"https://ice458.github.io/blog/{article_slug}/"
            permalink = f"/blog/{article_slug}/"
    ...
    def find_existing_article_directory(self, article_title):
        """記事タイトルに対応する既存の記事ディレクトリを検索"""
        blog_path = self.html_file.parent / "blog"

        if not blog_path.exists():
            return None

        # blog/article-* パターンのディレクトリをすべて検索
        for dir_path in blog_path.glob("article-*"):
            if dir_path.is_dir():
    ...
    def update_article_categories(self, article_title, categories):
        """記事ページのカテゴリを更新"""
        try:
            # 既存の記事ディレクトリを検索
            existing_dir = self.find_existing_article_directory(article_title)
            if not existing_dir:
                return False

            article_file = self.html_file.parent / "blog" / existing_dir / "index.html"
            ...
            # カテゴリ部分を置換（余計な改行を避ける）
            category_pattern = r'(<div class="article-categories">)(.*?)(</div>)'
            ...
            if new_content != content:
                with open(article_file, 'w', encoding='utf-8') as f:
                    f.write(new_content)
                print(f"記事カテゴリを更新: {article_title}")
                return True

            return True

        except Exception as e:
            print(f"記事ページのカテゴリ更新エラー ({article_title}): {e}")
            return False

    def update_all_article_categories(self):
        """全ての記事ページのカテゴリを現在の記事情報で更新"""
        updated_count = 0
        for article in self.articles:
            if self.update_article_categories(article['title'], article['categories']):
                updated_count += 1
